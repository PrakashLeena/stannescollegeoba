...
async function uploadImage(file: File) {
  ...
  fd.append('file', file)
  const res = await fetch('/api/admin/upload?folder=stannesppa/projects', { method: 'POST', body: fd })
    ...
    <div className="space-y-6">
      <div className="bg-white/10 border border-white/10 p-5">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="text-white font-semibold">{editingId ? 'Edit Project' : 'Add Project'}</div>
          {editingId && (
            <button
              type="button"
              disabled={saving}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            <label className="block text-white/80 text-sm mb-1">Summary</label>
            <textarea value={summary} onChange={(e) => setSummary(e.target.value)} className="w-full px-3 py-2 bg-black/20 border border-white/10 text-white outline-none" rows={2} />
          </div>
          <div className="sm:col-span-2">
      ...
      <div className="bg-white/10 border border-white/10">
        <div className="px-5 py-4 border-b border-white/10 text-white font-semibold">Projects</div>
        {loading ? (
          <div className="p-5 text-white/70">Loading...</div>
        ) : items.length === 0 ? (
          <div className="p-5 text-white/70">No projects yet.</div>
        ) : (
