...
async function uploadImage(file: File) {
  ...
  fd.append('file', file)
  const res = await fetch('/api/admin/upload?folder=stannesppa/members', { method: 'POST', body: fd })
...
export function MembersClient() {
  ...
  const canSubmit = useMemo(() => fullName.trim() && !saving, [fullName, saving])
  ...
  async function startEdit(id: string) {
    ...
    try {
      ...
      setEmail(it.email || '')
      setPhone(it.phone || '')
      setYearLeft(it.yearLeft || '')
      setJobTitle(it.jobTitle || '')
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(editingId ? `/api/admin/members/${editingId}` : '/api/admin/members', {
    ...
    <div className="space-y-6">
      <div className="bg-white/10 border border-white/10 p-5">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="text-white font-semibold">{editingId ? 'Edit Member' : 'Add Member'}</div>
          {editingId && (
            <button
              type="button"
              disabled={saving}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full px-3 py-2 bg-black/20 border border-white/10 text-white outline-none" />
          </div>
          <div>
            <label className="block text-white/80 text-sm mb-1">Year Left / Batch</label>
            <input value={yearLeft} onChange={(e) => setYearLeft(e.target.value)} className="w-full px-3 py-2 bg-black/20 border border-white/10 text-white outline-none" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} className="w-full px-3 py-2 bg-black/20 border border-white/10 text-white outline-none" />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            ...
            <textarea value={address} onChange={(e) => setAddress(e.target.value)} className="w-full px-3 py-2 bg-black/20 border border-white/10 text-white outline-none" rows={3} />
          </div>
        </div>
      ...
      <div className="bg-white/10 border border-white/10">
        <div className="px-5 py-4 border-b border-white/10 text-white font-semibold">Members</div>
        {loading ? (
          <div className="p-5 text-white/70">Loading...</div>
        ) : items.length === 0 ? (
          <div className="p-5 text-white/70">No members yet.</div>
        ) : (
              ...
              <tbody>
                {items.map((it) => (
                  <tr key={it._id} className="border-t border-white/10">
                    ...
                    <td className="px-4 py-3">{it.yearLeft || ''}</td>
